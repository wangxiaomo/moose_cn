...
Moose 拓展？
    ...
    编写拓展需要对 meta-model 有很好的理解，你可以通过 Moose::Manual::Mop 来了解 meta-model，在
    Moose::Cookbook 中你可以看到一些关于拓展的例子。
...
MooseX::StrictConstructor
    默认情况下，Moose 允许你添加代码到类的构造函数中。如果你加载了 MooseX::StrictConstructor 模块话，只要有
    Moose 无法识别的代码，Moose 就会抛出异常。
    ...
    加载 MooseX::StrictConstructor 后，错字（"emali"）将会引发异常，而默认情况下，这个错误是会被忽略的。

MooseX::Params::Validate
    我们对 MooseX::Method::Signatures 和 MooseX::Declare
    模块有很高的期望。然而在实际应用中仍然为测试版本。

    如果你不愿意为此冒险，我们推荐你使用略显笨重（但是更快、更简单）的 MooseX::Params::Validate 模块。这会让你使用
    Moose 的类型和类型约束来进行参数检验。
...
MooseX::Singleton
    ...
    在 Perl 中，你可以很方便的使用全局变量。另外，你也可以通过 MooseX::Singleton 来定义单态。
...
可供考虑的拓展
  ...
  MooseX::Types::Structured
    这个模块是在 MooseX::Types 的基础上创建，用来方便你声明较复杂的结构类型。
